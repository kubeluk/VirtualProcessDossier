...
- `WorkflowStepNode.vue` is a self-referencing recursive component for read-only display; **atomic nodes** render with a filled numbered circle, title, description, and system/input metadata; **composite nodes** render as anonymous structural containers — a hollow numbered circle + a type badge (`⟷ Parallel` / `↕ Sequential`) with a ▶/▼ collapse toggle; when collapsed the badge is followed by an activity count hint; sequential children render in a grey-bordered cluster, parallel children in a blue-bordered cluster
